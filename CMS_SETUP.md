...
- **서비스**: 서비스 카드, CTA 문구
- **제작 과정**: 단계별 이미지·체크리스트
- **포트폴리오**: 작품 추가/삭제, 카테고리, 썸네일, **YouTube URL**(권장) 또는 **MP4/WebM 업로드**(Supabase `media` 버킷, 50MB 이하)
- **YouTube 일괄 추가**: 관리자 → 포트폴리오 → 채널에서 복사한 `watch?v=` URL을 여러 줄로 붙여넣기 → 「목록에서 포트폴리오로 추가」→ **저장**
